  ...
  for ( int k = comeco; k < fim; k++ ) {
    ...
    for ( int j = 0; j <= qtyPages; j++ ) {
      ...
      try {
        ...
        log.close();
        log = null;
        log = createWriter(path + dia + ":" + mes + ":" + ano + "/_LOGGPA" + random(1,200) + "_" + dia + ":" + mes + ":" + ano + ".txt");
        wait(20 * 60 * 1000);    
        lines = loadStrings(partialAddress+j);
        e.printStackTrace();
      }
...
void BD_PaoDeAcucar() {
  ...
  if ( pgsql.connect() ) {
    ...
    for ( int i = 0; i < input2.length; i++ ) { 
      ...
      // }
      // catch(Exception e) {
      //   println("Exception no BANCO GPA. Esperar " + waitExceptionDB + " minutos.");
      //   log.println("Exception no BANCO GPA. Esperar " + waitExceptionDB + " minutos.");
